...
y_data = np.array([moc_vel_state_x, moc_vel_state_y, moc_vel_state_yaw])
...
u_data = np.array([moc_acc_state_x, input_state_steer])
...
# sol = fit_model.estimate(y_data, t_data, u_data, theta_0, bounds=([.1], [50.]))
# if sol.success is False:
#     print("PANIEK")
preds, ts = fit_model.f_predic(y_data, t_data, u_data, theta_0)
fit_model.f_resid(y_data, t_data, u_data, theta_0)

fig, ax = plt.subplots(nrows=2, ncols=2)
## plot longitudinal velocity
# ax[0][0].plot(t_data, moc_vel_state_x)
ax[0][0].plot(t_data[1:], y_data[0,1:])
ax[0][0].plot(t_data, moc_acc_state_x)
# ax[0][0].set_ylim([-5, 50])

## plot lateral velocity
# ax[0][1].plot(t_data, moc_vel_state_y)
ax[0][1].plot(t_data[1:], y_data[1,1:])
ax[0][1].plot(t_data, input_state_steer)
# ax[0][1].set_ylim([-5, 10])

## plot yaw rate
# ax[1][0].plot(t_data, encoder_state_vel)
ax[1][0].plot(t_data[1:], y_data[2,1:])
ax[1][0].plot(t_data, input_state_steer)
# ax[1][0].set_ylim([-5, 10])

# legends
ax[0][0].legend(["x vel", "acc input"])
ax[0][1].legend(["y vel", "steering input"])
ax[1][0].legend(["yaw rate", "steering input"])

for i in range(len(preds)):
    ax[0][0].plot(ts[i], preds[i][0,:])
    ax[0][1].plot(ts[i], preds[i][1,:])
    ax[1][0].plot(ts[i], preds[i][2,:])
